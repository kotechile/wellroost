...
| Accent | `#0072f5` | rgb(0, 114, 245) | hsl(212, 100%, 48%) | 3 |

### Neutral Colors

| Hex | HSL | Usage Count |
|-----|-----|-------------|
| `#171717` | hsl(0, 0%, 9%) | 2992 |
| `#ebebeb` | hsl(0, 0%, 92%) | 2409 |
| `#4d4d4d` | hsl(0, 0%, 30%) | 567 |
| `#fafafa` | hsl(0, 0%, 98%) | 121 |
| `#666666` | hsl(0, 0%, 40%) | 67 |
| `#000000` | hsl(0, 0%, 0%) | 50 |
| `#a8a8a8` | hsl(0, 0%, 66%) | 20 |
...
| Hex | Contexts | Count |
|-----|----------|-------|
| `#171717` | text, border, background | 2992 |
| `#ebebeb` | border, background | 2409 |
| `#4d4d4d` | text, border | 567 |
| `#fafafa` | background, text, border | 121 |
| `#666666` | text | 67 |
| `#000000` | border | 50 |
| `#a8a8a8` | border, text | 20 |
| `#7d7d7d` | text | 20 |
| `#8f8f8f` | background, text | 9 |
| `#297a3a` | text | 6 |
| `#0068d6` | text, background | 4 |
| `#52aeff` | background, text | 4 |
| `#0072f5` | text, background | 3 |
...
- **Geist** — used for all (2775 elements)
- **Geist Mono** — used for all (337 elements)
...
| 13px | 0.8125rem | 400 | 19.5px | normal | div, svg, path, ul |
...
`400` (2797x), `500` (255x), `600` (58x), `700` (2x)
...
```css
  ...
  border-radius: 0px;
  border: 0px 0px 2px none none solid rgb(77, 77, 77) rgb(77, 77, 77) rgba(0, 0, 0, 0);
  font-size: 14px;
  font-weight: 400;
```
...
**56 grid containers** and **501 flex containers** detected.
...
|----------|-----------|-------------|-------------|-----------|-------------|
| mobile (375px) | 16px | Yes | 8 | Yes | 9402px |
| tablet (768px) | 16px | Yes | 12 | Yes | 8844px |
| desktop (1280px) | 16px | Yes | 12 | Yes | 6401px |
| wide (1920px) | 16px | Yes | 12 | Yes | 5986px |

### Breakpoint Changes

**375px → 768px** (mobile → tablet):
- H1 size: `22px` → `28.8px`
- Max grid columns: `8` → `12`
- Page height: `9402px` → `8844px`

**768px → 1280px** (tablet → desktop):
- H1 size: `28.8px` → `48px`
- Page height: `8844px` → `6401px`

**1280px → 1920px** (desktop → wide):
- Page height: `6401px` → `5986px`
...
background-color: rgba(0, 0, 0, 0) → rgb(235, 235, 235);
border-color: rgb(77, 77, 77) → rgb(23, 23, 23);
...
background-color: rgba(0, 0, 0, 0) → rgb(235, 235, 235);
border-color: rgb(77, 77, 77) → rgb(23, 23, 23);
box-shadow: none → rgb(255, 255, 255) 0px 0px 0px 2px, rgb(0, 114, 245) 0px 0px 0px 4px;
outline: rgb(77, 77, 77) none 3px → rgb(23, 23, 23) none 3px;
```
...
**92 unique SVG icons** detected. Dominant style: **filled**.
...
**Icon colors:** `currentColor`, `rgb(0, 0, 0)`, `white`, `black`, `var(--ds-gray-1000)`, `url(#paint0_linear_53_108_r_j_)`, `url(#paint1_linear_53_108_r_j_)`, `red`, `url(#paint0_linear_53_108_r_k_)`, `url(#paint1_linear_53_108_r_k_)`
...
|---------|-------|------------|
| thumbnail | 9 | objectFit: fill, borderRadius: 0px, shape: square |
| gallery | 1 | objectFit: fill, borderRadius: 0px, shape: square |
| general | 1 | objectFit: fill, borderRadius: 8px, shape: rounded |

**Aspect ratios:** 2.79:1 (3x), 1:1 (2x), 3:2 (1x), 1.16:1 (1x), 4.78:1 (1x), 5.93:1 (1x), 3.66:1 (1x), 3.99:1 (1x)
...
|---|---|---|---|
| `globe-module__QBqKTa__blink` | fade | opacity | 3 |
...
- "start deploying" (2×)
- "get a demo" (2×)
- "deploy multi-tenant platforms" (2×)
- "enterprise" (2×)
- "security" (2×)
...
Reading order (top→bottom): feature-grid → hero → feature-grid → feature-grid → nav → nav → nav → nav → nav → feature-grid → feature-grid → hero → hero → feature-grid → hero → sidebar → nav → testimonial → feature-grid → feature-grid → footer → nav
...
| 13 | feature-grid | Scale your | 0.8 |
| 14 | hero | Deploy once, deliver everywhere. | 0.85 |
| 15 | hero | Fluid Compute | 0.85 |
| 16 | testimonial | AI Gateway | 0.8 |
...
**Label:** `gradient-mesh` (confidence 0.606)
**Counts:** total 11, svg 11, icon 3, screenshot-like 0, photo-like 0
...
|---------|---------|-----------|------|
| button--default | 0 | 101 × 30 | `screenshots/button-default-0.png` |
| button--default | 1 | 111 × 30 | `screenshots/button-default-1.png` |
| button--default | 2 | 104 × 30 | `screenshots/button-default-2.png` |
| button--outline--medium | 0 | 66 × 32 | `screenshots/button-outline-medium-0.png` |
| button--default--md | 0 | 386 × 48 | `screenshots/button-default-md-0.png` |
| button--default--xs | 0 | 48 × 50 | `screenshots/button-default-xs-0.png` |
| button--default--xs | 1 | 49 × 50 | `screenshots/button-default-xs-1.png` |
| button--default--xs | 2 | 88 × 50 | `screenshots/button-default-xs-2.png` |
| button--tertiary | 0 | 32 × 32 | `screenshots/button-tertiary-0.png` |
| card--default | 0 | 328 × 171 | `screenshots/card-default-0.png` |
| card--default | 1 | 328 × 171 | `screenshots/card-default-1.png` |
| card--default | 2 | 328 × 171 | `screenshots/card-default-2.png` |
